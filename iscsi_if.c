...
int
iscsi_control_recv_pdu(iscsi_cnx_h cp_cnx, struct iscsi_hdr *hdr, char *data, uint32_t data_size)
{
	...
	int retcode;
	uint64_t cnx;
	retcode = __session_cnx_recv_pdu(cp_cnx, hdr, data, data_size);
	

		if (retcode) {
			printf( "iscsi_control_recv_pdu: __session_cnx_recv_pdu done ! \n");
		} 
		else {
			printf ("iscsi_control_recv_pdu: __session_cnx_recv_pdu failed ! \n");
		}
