		...
		char *param, char *value)
{
	...
	if (text + length >= end) {
		//log_warning("Failed to add login text '%s=%s' \n", param, value);
		printf("Failed to add login text '%s=%s' \n", param, value);
		return 0;
	}
 ...
 */
int
iscsi_update_address(uiscsi_conn_t *conn, char *address)
{
	char *port;
	char *tag;
...
#ifdef SEC
static enum iscsi_login_status
get_security_text_keys(uiscsi_session_t *session, int cid, char **data,
...
static enum iscsi_login_status
check_security_stage_status(uiscsi_session_t *session,
...
static int
fill_security_params_text(uiscsi_session_t *session, int cid, struct iscsi_hdr *pdu,
...
static enum iscsi_login_status
check_for_authentication(uiscsi_session_t *session,
			 struct iscsi_acl *auth_client)
{
...
iscsi_login(uiscsi_session_t *session, int cid, char *buffer, size_t bufsize,
...
static enum iscsi_login_status
get_op_params_text_keys(uiscsi_session_t *session, int cid, char **data, char *end)
{
	char *text = *data;
	char *value = NULL;
	char *value_end = NULL;
	...
	else if (iscsi_find_key_value("TargetPortalGroupTag", text, end, &value, &value_end)) {

		printf("iscsi_find_key_value find TargetPortalGroupTag return %s\n", value);
	...
	else if (iscsi_find_key_value("MaxRecvDataSegmentLength", text, end, &value, &value_end)) {
		conn->max_xmit_dlength = strtoul(value, NULL, 0);
		text = value_end;
	} 
	else if (iscsi_find_key_value("FirstBurstLength", text, end, &value, &value_end)) {
		if (session->type == ISCSI_SESSION_TYPE_NORMAL)
	...
	else if (iscsi_find_key_value("MaxBurstLength", text, end, &value, &value_end)) {
		/*
 ...
 */
static enum iscsi_login_status
iscsi_process_login_response(uiscsi_session_t *session, int cid,
			     ...
			     char *data, int max_data_length)
{
	...
	//struct iscsi_acl *auth_client;
	uiscsi_conn_t *conn = &session->cnx[cid];
	printf("iscsi_process_login_response 1: conn->current_stage = %d\n",conn->current_stage);
	...
	if (end >= (data + max_data_length)) {
		printf ("Login failed, process_login_response buffer too small to guarantee NULL termination\n");
		return LOGIN_FAILED;
	}
	...
	*end = '\0';
	...
	if (login_rsp->status_class == ISCSI_STATUS_CLS_SUCCESS) {
		...
		{
			printf("Login version mismatch, received incompatible active iSCSI version 0x%02x, expected version 0x%02x", login_rsp->active_version,ISCSI_DRAFT20_VERSION);
			return LOGIN_VERSION_MISMATCH;
		}

		/* make sure the current stage matches */
		pdu_current_stage = (login_rsp->flags & ISCSI_FLAG_LOGIN_CURRENT_STAGE_MASK) >> 2;
		printf("iscsi_process_login_response 2: pdu_current_stage = 0x%x\n", pdu_current_stage);

		if (pdu_current_stage != conn->current_stage) {
			printf ("Received invalid login PDU, current stage mismatch, session %d, response %d", conn->current_stage,pdu_current_stage);
		 ...
		 */
		pdu_next_stage = login_rsp->flags & ISCSI_FLAG_LOGIN_NEXT_STAGE_MASK;
		printf("iscsi_process_login_response 3: pdu_next_stage = 0x%x\n", pdu_next_stage);
		printf("iscsi_process_login_response 4: transit = 0x%x\n", transit);
	...
	while (text && (text < end)) 
	{
		...
		case ISCSI_OP_PARMS_NEGOTIATION_STAGE:{
				ret = get_op_params_text_keys(session, cid, &text, end);
				if (ret != LOGIN_OK)
				{
					printf("iscsi_process_login_response 5: get_op_params_text_keys != LOGIN_OK, ret = %d\n", ret);
					return ret;					
				}
	...
	session->tsih = (login_rsp->tsih);
	session->exp_cmdsn =(login_rsp->exp_cmdsn);
	session->max_cmdsn =(login_rsp->max_cmdsn);
	printf("iscsi_process_login_response: session->max_cmdsn =(login_rsp->max_cmdsn) = %d!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");

	if (login_rsp->status_class == ISCSI_STATUS_CLS_SUCCESS)
	{
		conn->exp_statsn =(login_rsp->statsn) + 1;
		printf("!!!!!!!!!!!!!!!!!!!!!!!!iscsi_process_login_response: conn->exp_statsn =(login_rsp->statsn) + 1;    !!!!!!!!!!!!!!!!!!!!!!!! ");
	}
	...
	if (transit) 
	{
		...
		conn->current_stage = login_rsp->flags & ISCSI_FLAG_LOGIN_NEXT_STAGE_MASK;
	...
	printf("iscsi_process_login_response 6: conn->current_stage = %d\n",conn->current_stage);
...
static int
add_params_normal_session(uiscsi_session_t *session, struct iscsi_hdr *pdu,
                    char *data, int max_data_length)
{
	char value[256];

	/* these are only relevant for normal sessions */
	//if (!iscsi_add_text(pdu, data, max_data_length, "InitialR2T", session->initial_r2t_en ? "Yes" : "No"))
	if (!iscsi_add_text(pdu, data, max_data_length, "InitialR2T",  "No"))	
		return 0;

	//if (!iscsi_add_text(pdu, data, max_data_length, "ImmediateData", session->imm_data_en ? "Yes" : "No"))
	if (!iscsi_add_text(pdu, data, max_data_length, "ImmediateData", "Yes" ))
		return 0;
...
static int
add_vendor_specific_text(uiscsi_session_t *session, int cid, struct iscsi_hdr *pdu,
                    char *data, int max_data_length)
{
	char value[256];
	uiscsi_conn_t *conn = &session->cnx[cid];
...
static int
check_irrelevant_keys(uiscsi_session_t *session, struct iscsi_hdr *pdu,
                    char *data, int max_data_length)
{
...
static int
fill_crc_digest_text(uiscsi_conn_t *conn, struct iscsi_hdr *pdu,
		     char *data, int max_data_length)
{
...
static int
fill_op_params_text(uiscsi_session_t *session, int cid, struct iscsi_hdr *pdu,
		    char *data, int max_data_length, int *transit)
{
	...
	*transit = 1;
	...
	if (!conn->partial_response) 
	{
		...
		if (!iscsi_add_text(pdu, data, max_data_length, "MaxRecvDataSegmentLength", value))
			return 0;

		sprintf(value, "%d", session->def_time2wait);
		if (!iscsi_add_text(pdu, data, max_data_length, "DefaultTime2Wait", value))
			return 0;

		//sprintf(value, "%d", session->def_time2retain);
		if (!iscsi_add_text(pdu, data, max_data_length, "DefaultTime2Retain", value))
			return 0;

		if (!iscsi_add_text(pdu, data, max_data_length, "IFMarker", "No"))
			return 0;

		if (!iscsi_add_text(pdu, data, max_data_length, "OFMarker", "No"))
			return 0;

		if (!iscsi_add_text(pdu, data, max_data_length, "ErrorRecoveryLevel", "0"))
			return 0;
		...
		//	if (!add_vendor_specific_text(session, cid, pdu, data, max_data_length))
		//		return 0;
	} 
 ...
 **/
static int
iscsi_make_login_pdu(uiscsi_session_t *session, int cid, struct iscsi_hdr *hdr,
		     char *data, int max_data_length)
{
	int transit = 0;
	int ret;
	...
	//auth_client = (session->auth_buffers && session->num_auth_buffers) ? (struct iscsi_acl *)session->auth_buffers[0].address : NULL;
	...
	// ?
	login_hdr->exp_statsn = conn->exp_statsn;
	...
	if (conn->current_stage == ISCSI_INITIAL_LOGIN_STAGE) 
	{
		...
		{
			if (!iscsi_add_text(hdr, data, max_data_length, "InitiatorName", session->initiator_name))
				return 0;
		} 
		else 
		{
			printf("InitiatorName is required on the first Login PDU\n");
			return 0;
		}
		
		if (session->initiator_alias && session->initiator_alias[0]) 
		{
			//if (!iscsi_add_text(hdr, data, max_data_length,"InitiatorAlias", session->initiator_alias))
			if (!iscsi_add_text(hdr, data, max_data_length,"InitiatorAlias", session->initiator_alias))
				return 0;
		}
		...
		{
			if (!iscsi_add_text(hdr, data, max_data_length,"TargetName", session->target_name))
				return 0;
		}

		if (!iscsi_add_text(hdr, data, max_data_length, "SessionType", (session->type ==ISCSI_SESSION_TYPE_DISCOVERY) ? "Discovery" : "Normal"))
			return 0;
	...
	case ISCSI_OP_PARMS_NEGOTIATION_STAGE:
		{
			ret = fill_op_params_text(session, cid, hdr, data, max_data_length, &transit);
			if (!ret)
				return ret;
			break;
		}
	...
	case ISCSI_FULL_FEATURE_PHASE:
		printf("Can't send login PDUs in full feature phase\n");
		return 0;
	default:
		printf("Can't send login PDUs in unknown stage %d\n", conn->current_stage);
		return 0;
	}
...
static enum iscsi_login_status
check_status_login_response(uiscsi_session_t *session, int cid,
			    struct iscsi_login_rsp *login_rsp,
			    char *data, int max_data_length, int *final)
{
	...
	case ISCSI_STATUS_CLS_SUCCESS:
		printf("check_status_login_response: ISCSI_STATUS_CLS_SUCCESS\n");
		/* process this response and possibly continue sending PDUs */
		ret = iscsi_process_login_response(session, cid, login_rsp, data, max_data_length);
	...
	case ISCSI_STATUS_CLS_REDIRECT:
		printf("check_status_login_response: ISCSI_STATUS_CLS_REDIRECT\n");
		/*
		 * we need to process this response to get the
		 * TargetAddress of the redirect, but we don't care
		 * about the return code.
		 */
		iscsi_process_login_response(session, cid, login_rsp, data, max_data_length);
		
		ret = LOGIN_OK;
		*final = 1;
	case ISCSI_STATUS_CLS_INITIATOR_ERR:
		printf("check_status_login_response: ISCSI_STATUS_CLS_INITIATOR_ERR\n");
		if (login_rsp->status_detail ==ISCSI_LOGIN_STATUS_AUTH_FAILED) 
		{
			printf("Login failed to authenticate with target %s\n", session->target_name);
		}
		ret = LOGIN_OK;
		*final = 1;
	default:
...
int
iscsi_login_begin(uiscsi_session_t *session, iscsi_login_context_t *c)
{
	uiscsi_conn_t *conn = &session->cnx[c->cid];
...
// 1 Ê§°Ü
int
iscsi_login_req(uiscsi_session_t *session, iscsi_login_context_t *c)
{
	uiscsi_conn_t *conn = &session->cnx[c->cid];
	...
	if (!iscsi_make_login_pdu(session, c->cid, &c->pdu, c->data,
				  c->max_data_length)) 
	{
		printf("login failed, couldn't make a login PDU\n");
		c->ret = LOGIN_FAILED;
		goto done;
	}
			    ...
			    c->data, ISCSI_DIGEST_NONE, c->timeout)) 
	{
		 ...
		 */
		printf("Login I/O error, failed to send a PDU\n");
...
// ³É¹¦·µ»Ø0 £¬ Ê§°Ü·µ»Ø1
//int iscsi_login_rsp(uiscsi_session_t *session, iscsi_login_context_t *c)
int iscsi_login_rsp(uiscsi_session_t *session, iscsi_login_context_t *c, struct iscsi_hdr *hdr, char *data, uint32_t data_size)	
{
	uiscsi_conn_t *conn = &session->cnx[c->cid];

	/* read the target's response into the same buffer */
	//iscsi_recv_pdu(conn, &c->pdu, char * data, int max_data_length);

	//&c->pdu = hdr;
	memcpy(&c->pdu, hdr, sizeof(struct iscsi_hdr));
	c->data = data;
	c->login_rsp = (struct iscsi_login_rsp *)&c->pdu;

	if (!iscsi_recv_pdu(conn, &c->pdu, ISCSI_DIGEST_NONE, c->data, data_size, c->max_data_length, ISCSI_DIGEST_NONE, c->timeout)) 
	{
		/*
		 * FIXME: caller might want us to distinguish I/O
		 * error and timeout. Might want to switch portals on
		 * timeouts, but not I/O errors.
		 */
		printf("Login I/O error, failed to receive a PDU \n");
		c->ret = LOGIN_IO_ERROR;
		goto done;
	}
	...
	if (c->pdu.opcode == (ISCSI_OP_LOGIN_RSP | 0xC0)) 
	{
		 ...
		 */
		printf("iscsi_login_rsp: Received iSCSI draft 8 login response opcode 0x%x, expected draft 20 login response 0x%2x \n ", c->pdu.opcode, ISCSI_OP_LOGIN_RSP);
	...
	c->ret = check_status_login_response(session, c->cid, c->login_rsp, c->data, c->max_data_length, &c->final);

	printf("iscsi_login_rsp: c->ret = check_status_login_response   = %d\n", c->ret);
	printf("iscsi_login_rsp: conn->current_stage = %d\n", conn->current_stage);
	if (c->final)
		goto done;
